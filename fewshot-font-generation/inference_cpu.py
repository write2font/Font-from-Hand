...
def infer_DM(gen, save_dir, gen_chars, key_ref_dict, load_img, decomposition, batch_size=32, return_img=False):
    ...
    for key, gchars in key_gen_dict.items():
        ...
        for batch, rchars in zip(ref_batches, ref_chars):
            decs = torch.LongTensor([decomposition[c] for c in rchars]).to(DEVICE)
        ...
        for char in gchars:
            dec = torch.LongTensor([decomposition[char]]).to(DEVICE)
            fid = [0]
...
def infer_LF(gen, save_dir, source_path, source_ext, gen_chars, key_ref_dict, load_img, decomposition, primals, batch_size=32, return_img=False):
    ...
    for key, gchars in key_gen_dict.items():
        ...
        for batch, rchars in zip(ref_batches, ref_chars):
            ...
            decs = torch.LongTensor(list(chain(*decs))).to(DEVICE)
            batch = batch.repeat_interleave(dec_lens, dim=0).to(DEVICE)
        ...
        for char in gchars:
            source_dec = torch.LongTensor(decompose(char)).to(DEVICE)
            source_img = torch.stack([TRANSFORM(read_source(char))] * len(source_dec)).to(DEVICE)
...
def infer_MX(gen, save_dir, source_path, source_ext, gen_chars, key_ref_dict, load_img, batch_size=32, return_img=False):
    ...
    for key, gchars in key_gen_dict.items():
        ...
        for char in gchars:
            source_img = TRANSFORM(read_source(char)).unsqueeze(0).to(DEVICE)
...
def load_model(args, cfg, gen_model):
    ...
    # [수정] 모델을 현재 DEVICE (CPU 또는 GPU)로 올립니다.
    gen = gen_model(**g_kwargs).to(DEVICE)
    
    # [수정] 가중치를 불러올 때 map_location을 DEVICE로 지정합니다.
    weight = torch.load(args.weight, map_location=DEVICE, weights_only=False)
