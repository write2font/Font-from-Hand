...
{transcript_text[:2000]}"""
    ...
    def _generate_cover_title(self, transcript_text, keywords, user_name, birth_year,
        ...
        prompt = (
            ...
            f"핵심 소재: {kw_str}\n\n"
            f"좋은 제목 예시 (짧고 구체적인 명사·장소·순간 중심):\n"
            f"- 그 주차장에서\n"
            f"- 축구공 하나\n"
            f"- 기숙사 창문 너머\n"
            f"- 흙 묻은 운동화\n"
            f"- 냇가의 여름\n\n"
            f"나쁜 제목 (금지):\n"
            f"- 길고 설명적인 것 (공을 차며 쌓아 올린 나날들, 성실하게 살아온 인생)\n"
            f"- 추상적인 것 (삶의 여정, 인생의 발자취, 나의 이야기)\n"
            f"- 서술어·조사 과다 (~하며, ~쌓아 올린, ~보내온)\n\n"
            f"규칙: 한국어만, 4~10자, 구체적 명사나 장소 중심. 제목 하나만 출력."
        )
        try:
            response = client.chat.completions.create(
                model=config.LLM_MODEL,
    ...
    def _generate_chapter(self, chapter, title, transcript_text, keywords,
        ...
        system_prompt = (
            ...
            "【독자를 위한 자서전 글쓰기】\n"
            "- 첫 문단: 요약 문장 대신 그날의 소리·냄새·풍경 같은 구체적 장면으로 시작하라.\n"
            "- 이 사람을 전혀 모르는 독자가 읽는다고 가정하고, 배경과 상황을 자연스럽게 녹여라.\n"
            "- 사건의 나열이 아니라, 그 경험이 왜 기억에 남는지·어떤 감정이었는지를 중심으로 써라.\n"
            "- 마지막 문단: 그 시절이 지금의 자신에게 어떤 의미인지로 마무리하라."
        )
