...
class FollowupQuestioner:
    ...
    def _generate_followups(self, segments: list) -> list:
        ...
        prompt = (
            ...
            f"{thin_str}\n\n"
            f"각 항목에 대해 더 풍부한 이야기를 끌어낼 수 있는 2차 질문을 만들어라.\n"
            f"질문은 아래 순서 중 답변에서 빠진 부분을 채우는 방향으로 만들어라:\n"
            f"  배경(그때 상황) → 사건(구체적으로 무슨 일이) → 감정(그때 어떤 기분이었는지) → 의미(지금 돌아보면)\n\n"
            f"【필수 규칙】\n"
            f"1. 답변에 언급되지 않은 내용을 가정하거나 추측하는 질문 절대 금지.\n"
            f"2. 답변에 있는 단어나 사실만을 기반으로 질문을 만들어라.\n"
            f"3. 비슷한 질문 중복 금지. 항목마다 최대 1개.\n"
            f"4. 답변이 아예 없으면('(없음)') 그 시기의 배경이나 감정을 묻는 열린 질문으로 만들어라.\n\n"
            f"형식 (각 줄):\n"
            f"Q번호|기존 질문 요약|2차 질문\n"
            f"예시:\n"
            f"Q3|좋아했던 장소|그곳에서 혼자 있을 때 주로 어떤 생각을 하셨나요?\n"
            f"Q5|어린 시절 꿈|그 꿈을 처음 갖게 됐을 때 기분이 어땠는지 기억하세요?\n\n"
